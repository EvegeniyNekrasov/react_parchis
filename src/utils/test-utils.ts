...
export const hexToRgba = (hex: string, alpha = 1): string => {
  ...
  if (hex.length === typeOfHex.SHORT) {
    ({
      ...
      green: g,
      blue: b,
    } = createRgbFromShordHexadecimal(hexodecimal, RADIX));
  } else if (hex.length === typeOfHex.FULL) {
    ({
      red: r,
      green: g,
      blue: b,
    } = createRgbFromFullHexadecimal(hexodecimal, RADIX));
  } else {
    throw new Error(
      "Invalid hex color format, the length of the color must be 3 or 6 digits after #",
    );
  }
...
const createRgbFromShordHexadecimal = (hex: string, radix: number): Rgb => {
  const red = Number.parseInt(
    hex.charAt(hexChartPosition.START) + hex.charAt(hexChartPosition.START),
    radix,
  );
  const green = Number.parseInt(
    hex.charAt(hexChartPosition.MIDDLE) + hex.charAt(hexChartPosition.MIDDLE),
    radix,
  );
  const blue = Number.parseInt(
    hex.charAt(hexChartPosition.END) + hex.charAt(hexChartPosition.END),
    radix,
  );
...
const createRgbFromFullHexadecimal = (hex: string, radix: number): Rgb => {
  const red = Number.parseInt(
    hex.substring(hexSubString.ZERO, hexSubString.TWO),
    radix,
  );
  const green = Number.parseInt(
    hex.substring(hexSubString.TWO, hexSubString.FOUR),
    radix,
  );
  const blue = Number.parseInt(
    hex.substring(hexSubString.FOUR, hexSubString.SIX),
    radix,
  );
