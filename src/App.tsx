...
function App() {
  ...
  const handlePlayersCount = (e: React.ChangeEvent<HTMLInputElement>) => {
    setPlayersCount(Number.parseInt(e.target.value));
    const p = players.slice(0, Number.parseInt(e.target.value));
    setPlayersData(p);
  };
  ...
  return (
    <div className="main">
      ...
      {/* TODO: add diferent interface to ParchisBoard */}
      {ready ? <ParchisBoard players={playersData} index={0} /> : null}
    </div>
  );
}
